...
svdrpclient.prototype.scanEPG = function( callback ) {
  this.runSimpleCommand( 'SCAN', callback );
};
...
svdrpclient.prototype.deleteTimer = function( callback, timerId ) {
  this.runSimpleCommand( 'DELT ' + timerId, callback );
};
...
svdrpclient.prototype.diskStats = function( callback ) {
  var self = this;
  this.runCommand( 'STAT disk', {}, function( result ) {
    var stats = {};
    var parts = result.data.pop().data.split(/ /);
    stats.total = parseInt( parts[0] );
    stats.free = parseInt( parts[1] );
    stats.percent = parseInt( parts[2] );
    result.data = stats;
    callback( result );
  });
};

svdrpclient.prototype.changeRemote = function( callback, status ) {
  var self = this;
  var command = 'REMO';
  if( status ) {
    command += ' ' + status;
  ...
  this.runCommand( command, {}, function( result ) {
    var stats = {};
    if( result.code === '250' ) {
      if( result.data.pop().data.match(/enabled/) ) {
        result.data = true;
      } else {
        result.data = false;
      }
...
svdrpclient.prototype.deleteRecording = function( callback, recordingId ) {
  var command = 'DELR ' + recordingId;
  this.runSimpleCommand( command, callback );
};

svdrpclient.prototype.updateRecordings = function( callback ) {
  this.runSimpleCommand( 'UPDR', callback );
};

svdrpclient.prototype.displayMessage = function( callback, message ) {
  var command = 'MESG ' + message;
  this.runSimpleCommand( command, callback );
};

svdrpclient.prototype.runSimpleCommand = function( command, callback ) {
  var self = this;
  this.runCommand( command, {}, function( result ) {
    callback( result );
  });
...
svdrpclient.prototype.runCommand = function( cmd, params, callback ) {
  var self = this;
  cmd = this.cleanCommand(cmd);
  var timeout;


  try {
    var client = net.connect( this.options.port, this.options.host,
    ...
    client.setTimeout( this.options.connectionTimeout || this.options.timeout );

    timeout = setTimeout( function() {
      client.destroy();
      callback( { code : "903", data : "No complte response within timeout" } );
    }, this.options.responseTimeout || this.options.timeout );

    client.on('error', function (error) {
      client.destroy();
      callback( { code : "901", data : error.code } );
    });
    client.on('timeout', function () {
      client.destroy();
      callback( { code : "902", data : '' } );
    });
  } catch( error ) {
    callback( { code : "901", data : error.code } );
  }
...
svdrpclient.prototype.cleanCommand = function( command ) {
  var re = /\r?\n\r?/;
  var parts = command.split(re);
  return parts[0];
}
...
svdrpclient.prototype.executeCommand = function( client, cmd, params, callback ) {
  ...
  client.on('data', function(chunk) {
    ...
    if( lastfound && result.code == 220 ) {
      // execute the command once we've got the vdr banner
      client.write( cmd + '\n');
    }
    else if( data === '' && lastfound ) {
      // if the last line was empty, and we've got a space after the response
      // code, indicating the last line, were done
      if( !resultSent ) {
        // make sure we're not calling the callback more than once, just in case
